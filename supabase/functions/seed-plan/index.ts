...
const EASY_MIN = 480
const EASY_MAX = 523
const TEMPO_PACE = 420
const INTERVAL_PACE = 386
...
const tempo = (week: number, day: number, min: number): Row => {
  const sec = min * 60
  const km = Math.round((sec / TEMPO_PACE) * 100) / 100
  return {
    week,
    day,
    type: "tempo",
    ...
    pace_min: TEMPO_PACE,
    pace_max: TEMPO_PACE,
    description: `${min} minutes continuous at threshold ("comfortably hard") effort, about 7:00/km.`
  }
}
...
const interval = (week: number, day: number, reps: number): Row => ({
  ...
  title: `${reps}×400m`,
  distance_m: reps * 400,
  pace_min: INTERVAL_PACE,
  pace_max: INTERVAL_PACE,
  steps: [
    { label: "400m repeat", repeat: reps, distance_m: 400, pace_sec_per_km: INTERVAL_PACE, recovery: "400m easy jog/walk" }
  ],
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await supabase
      .from("profiles")
      .update({ threshold_pace_sec_per_km: TEMPO_PACE, units: "metric" })
      .eq("id", userId)
      .is("threshold_pace_sec_per_km", null)

    return new Response(JSON.stringify({ ok: true, plan_id: plan.id }), {
      headers: { ...CORS, "Content-Type": "application/json" }
    })
  } catch (e) {
